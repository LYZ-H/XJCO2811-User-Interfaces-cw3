...
void ResponsiveLayout::portraitMode(QFrame *frame, QScrollArea *qw, QVideoWidget *videoWidget,
                                    QSlider *videoSlider, const QRect &r) {
    if(qw) {
        qw->setGeometry(0 + r.x(), 0.6 * r.height() + 96 + r.y(), r.width(), 0.4 * r.height() -96);
    } else if (videoWidget) {
        videoWidget->setGeometry(0 + r.x(), 0, r.width(), 0.6 * r.height() - 10);
    } else if (videoSlider) {
        videoSlider->setGeometry(r.x() + 10, 0.6 * r.height() + 2, r.width() - 140, 30);
    } else if (frame->whatsThis() == "length_label") {
        frame->setGeometry(r.width() - 115, 0.6 * r.height() + 2, 60, 30);
    } else if (frame->whatsThis() == "duration_label") {
        frame->setGeometry(r.width() - 57, 0.6 * r.height() + 2, 60, 30);
    } else if (frame->whatsThis() == "buttons") {
        frame->setGeometry(0.5 * r.width() + r.x() - 131, 0.6 * r.height() + 30, 265, 60);
    }
}
