        ...
        if (f.contains("."))
            ...
            if (f.contains(".wmv")) { // windows
                ...
                if (QFile(thumb).exists()) { // if a png thumbnail exists
                    ...
                    if (!sprite.isNull()) {
                        auto *ico = new QIcon(QPixmap::fromImage(sprite)); // voodoo to create an icon for the button
                        QUrl *url = new QUrl(QUrl::fromLocalFile(f)); // convert the file location to a generic url
                        out.emplace_back(url, ico); // add to the output list
                    } else
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        TheButton::connect(button, SIGNAL(jumpTo(TheButtonInfo * )), player,
